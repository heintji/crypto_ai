...
# ============================================================
# Zelfde gedachtegang als whatsapp_webhook.py + live_trader.py:
#
...
#   ✅ Zelfde DB logging naar experience_trades (source=LIVE)
...
#   ✅ Zelfde weekend: gewoon bewaken — nooit blokkeren
#
# BUGS GEFIXED vs origineel:
#   ✅ send_whatsapp niet gedefinieerd → crashte bij elke SELL
#   ✅ Dubbele functiedefinities → eerste versie werd genegeerd
#   ✅ below_1r_count telde monitor-runs ipv candles
#   ✅ Geen DB logging van live trade closes
#   ✅ State werd altijd opgeslagen ook als er niets veranderde
#
# NIEUWE FEATURES:
...
#   ✅ Coin cooldown check (24u na verlies op die coin)
#   ✅ Rolling 30-dagen metrics
#   ✅ Coin blacklist (winrate <30% na 20 trades)
...
# ============================================================
# ENV — identiek aan whatsapp_webhook.py + live_trader.py
# ============================================================
...
# Zelfde patroon als alle andere bestanden
# ============================================================
...
def report_error(
    ...
    Zelfde ernst niveaus als alle andere bestanden:
...
Er is een fout in trade_monitor.py opgetreden.
...
""".strip()
    ...
    send_whatsapp(
        ...
        f"⚠️ Fout type:   {type(error).__name__}\n\n"
        f"🧠 Claude analyse:\n"
        f"{uitleg}\n\n"
        f"📋 WAT JE KUNT DOEN:\n"
...
def claude_analyseer_gesloten_trade(
    ...
    """
    Claude analyseert elke gesloten trade.
    Geeft korte tekst terug — wordt NIET via WhatsApp gestuurd (spam).
...
Was de exit logisch? Wat leren we hiervan?
""".strip()
...
# ============================================================
# DB LOGGING — identiek aan live_trader.py
# Naar experience_trades met source=LIVE
# ============================================================
def update_trade_in_db(
    ...
    """
    Update live trade in experience_trades naar WIN of LOSS.
    Zelfde tabel als webhook rapportage gebruikt.
    """
    try:
        ...
        with conn.cursor() as cur:
            ...
            """, (
                ...
                exit_price, outcome, pnl_eur, score,
                exit_time,
            ))

            # Sla Claude analyse op als aparte notitie
            if claude_analyse:
                cur.execute(f"""
                INSERT INTO {BOT_STATE_TABLE}(key, value, updated_at)
                VALUES (%s, %s, NOW())
                ON CONFLICT(key) DO UPDATE
                    SET value=EXCLUDED.value, updated_at=NOW()
                """, (
                    f"trade_analyse_{symbol}_{int(exit_time.timestamp())}",
                    json.dumps({
                        "symbol":    symbol,
                        "outcome":   outcome,
        ...
        conn.commit()
        log(f"✅ DB bijgewerkt: {symbol} {outcome} €{pnl_eur:.4f}")
    except Exception as e:
        log(f"⚠️ DB update fout ({symbol}): {type(e).__name__}: {e}")
...
def get_rolling_stats(conn, days: int = 7) -> Tuple[int, int, float]:
    ...
    try:
        with conn.cursor() as cur:
            ...
            WHERE UPPER(COALESCE(source,'')) IN ('REAL','LIVE')
              AND UPPER(COALESCE(outcome,'')) IN ('WIN','LOSS')
              AND COALESCE(exit_time, updated_at) >= NOW() - INTERVAL '%s days'
            """, (days,))
            row = cur.fetchone()
            if row:
                return safe_int(row[0]), safe_int(row[1]), safe_float(row[2])
...
def check_edge_decay(conn) -> Optional[str]:
    ...
    Als verschil >10% = edge decay mogelijk.
    Geeft waarschuwingsbericht terug of None.
    """
    try:
        with conn.cursor() as cur:
            cur.execute("""
        ...
        if diff > EDGE_DECAY_THRESHOLD and sim_wr > 0 and real_wr > 0:
            return (
                ...
                f"💡 WAT DIT BETEKENT:\n"
                f"De strategie presteert live anders\n"
                f"dan in simulatie. Dit kan komen\n"
                f"door marktveranderingen, fees,\n"
                f"slippage of timing verschillen.\n\n"
                f"📋 MOGELIJKE OORZAKEN:\n"
                f"• Markt is veranderd (regime shift)\n"
                f"• Fees/slippage niet meegenomen in sim\n"
                f"• Entry/exit timing verschilt live\n"
                f"• Score drempel te laag\n\n"
                f"🤖 BOT LOOPT GEWOON DOOR\n"
                f"Claude health check aanbevolen.\n"
                f"Stuur HEALTH voor analyse.\n\n"
...
def is_coin_on_cooldown(conn, symbol: str) -> bool:
    """
    Controleert of coin in cooldown zit na een verlies.
    Cooldown: 24 uur na laatste verlies op die coin.
    """
    try:
        with conn.cursor() as cur:
            cur.execute("""
...
def is_coin_blacklisted(conn, symbol: str) -> bool:
    """
    Controleert of coin op de blacklist staat.
        ...
        with conn.cursor() as cur:
            ...
            if row:
                ...
                if n >= BLACKLIST_MIN_TRADES:
                    wr = wins / n
                    return wr < BLACKLIST_MAX_WINRATE
    except Exception:
        pass
    return False
...
def save_state(state: Dict[str, Any], force: bool = False) -> None:
    """
    Slaat state op naar live_state.json.
    Alleen als er iets veranderd is (of force=True).
    Fix: state werd altijd opgeslagen ook bij geen wijzigingen.
    """
    _ensure_dir(LIVE_STATE_PATH)
...
# ============================================================
# PRIJS OPHALEN via Binance (geen auth nodig)
# ============================================================
def get_current_price_binance(symbol_usdt: str) -> Optional[float]:
    """Haalt huidige prijs op via Binance public API."""
    try:
        url  = f"https://api.binance.com/api/v3/ticker/price?symbol={symbol_usdt.upper()}"
...
def get_current_price_bitvavo(market: str) -> Optional[float]:
    """Haalt huidige prijs op via Bitvavo public API."""
    try:
...
def get_price(symbol_usdt: str, market: str) -> Optional[float]:
    """Probeert Bitvavo eerst, dan Binance als fallback."""
...
def _execute_sell(
    ...
    try:
        ...
        log(f"❌ Sell import/call fout ({symbol}): {type(e).__name__}: {e}")
...
def _calc_r(entry: float, stop: float, current: float) -> float:
    """Berekent R-multiple. Positief = in de richting van target."""
    risk = abs(entry - stop)
    if risk <= 0:
        return 0.0
...
def _holding_minutes(trade: Dict[str, Any]) -> float:
    ...
    opened_at = safe_float(trade.get("opened_at") or trade.get("monitor_started_at"))
    if not opened_at:
        return 0.0
...
def process_live_trade(
    ...
    # 1. Max houdtijd (48u) — altijd controleren
    if hold_min >= MAX_HOLD_HOURS * 60:
        log(f"⏰ {symbol}: max houdtijd bereikt ({hold_min:.0f}min) → SELL 100%")
    ...
    # 2. Stop loss — R < 0 én prijs onder stop
    if current <= stop or r < 0:
        log(f"🛑 {symbol}: stop geraakt (prijs={current:.6f} stop={stop:.6f}) → SELL 100%")
        result = _execute_sell(symbol, 1.0, meta={"exit_reden": "STOP_LOSS"})
        _finalize_trade(symbol, trade, current, result, conn, "STOP_LOSS")
        return True, result.get("ok", False)

    # 3. Target/structuur modus
    if target > 0 and current >= target:
        if not trade.get("target_reached_notified"):
            trade["target_reached_notified"] = True
            trade["mode"] = "STRUCTUUR"
            trade["structuur_low"] = current
            log(f"🎯 {symbol}: target bereikt @ {current:.6f} → STRUCTUUR mode")
            changed = True

    # 4. Structuur modus — trailing exit
    if trade.get("mode") == "STRUCTUUR":
        structuur_low = safe_float(trade.get("structuur_low"), current)

        # Update structuur low
        if current > structuur_low:
            trade["structuur_low"] = current
            changed = True
        elif current < structuur_low * 0.99:
            # Prijs breekt 1% onder structuur low → exit
    ...
    # 5. Partial sell na eerste keer >1R
    if r >= 1.0 and not trade.get("had_over_1r"):
        ...
        changed = True
        log(f"📈 {symbol}: eerste keer >1R (R={r:.2f}) — nog niet verkopen")

    # 6. Terug onder 1R na had_over_1r → partial sell 40%
    ...
    # 7. Telt candle-bars <1R na partial sell (FIX: telt nu candles niet monitor-runs)
    if trade.get("partial_sold_40") and r < 1.0:
        # Gebruik held_candles als proxy voor candle count
        candle_period_min = 4 * 60  # 4H candle = 240 min
        timeframe = safe_str(trade.get("timeframe"), "4h")
        if "1h" in timeframe.lower():
            candle_period_min = 60
        elif "15m" in timeframe.lower():
            candle_period_min = 15

        # Verhoog teller alleen als er een volledige candle is verstreken
        last_candle_check = safe_float(trade.get("last_candle_check_ts"))
        now_ts = time.time()

        if now_ts - last_candle_check >= candle_period_min * 60:
    ...
    if entry_price > 0:
        mfe_r = (safe_float(trade.get("max_price_seen"), current) - entry_price) / max(abs(entry_price - stop), 0.000001)
        mae_r = (entry_price - safe_float(trade.get("min_price_seen"), current)) / max(abs(entry_price - stop), 0.000001)
    ...
    if current > safe_float(trade.get("max_price_seen"), current):
        ...
        changed = True
    if current < safe_float(trade.get("min_price_seen"), current) or not trade.get("min_price_seen"):
...
def _finalize_trade(
    ...
    fee_eur    = safe_float(sell_result.get("fee_eur"), _calc_fee(exit_price * safe_float(trade.get("qty", 0))))
    ...
    log(f"📊 {symbol} gesloten: {outcome} €{pnl_eur:.4f} | R={exit_r:.2f} | MFE={mfe_r:.2f}R MAE={mae_r:.2f}R | {exit_reden}")

    # Claude analyse per trade
    ...
    update_trade_in_db(
        ...
        fee_eur, claude_tekst,
    )

    # Consecutive loss informatie — bot stopt NIET automatisch
    # Alleen jij kunt de bot stoppen via WhatsApp STOP command
    consecutive = get_consecutive_losses(conn)
    if consecutive >= MAX_CONSECUTIVE_LOSSES:
        # Haal extra context op voor het bericht
        wins_30d, losses_30d, pnl_30d = get_rolling_stats(conn, days=30)
        wins_7d, losses_7d, pnl_7d    = get_rolling_stats(conn, days=7)
        total_7d  = wins_7d + losses_7d
        wr_7d     = (wins_7d / total_7d * 100) if total_7d > 0 else 0.0
        total_30d = wins_30d + losses_30d
        wr_30d    = (wins_30d / total_30d * 100) if total_30d > 0 else 0.0
        pf_30d    = get_profit_factor(conn, days=30)
        ...
        send_whatsapp(
            ...
            f"📊 WIN RATE VANDAAG:\n"
            f"• Wins:   {wins_7d} | Losses: {losses_7d}\n"
            f"• Win rate 7d:  {wr_7d:.1f}%\n"
            f"• Win rate 30d: {wr_30d:.1f}%\n"
            f"• PnL vandaag: €{pnl_7d:.2f}\n\n"
            f"📈 PROFIT FACTOR 30d: {pf_30d:.2f}\n"
            f"{'✅ Gezond' if pf_30d >= 1.5 else '⚠️ Let op — doel is >1.5'}\n\n"
            f"🤖 BOT LOOPT GEWOON DOOR\n"
    ...
    try:
        ...
        if trade_count_30d > 0 and trade_count_30d % 10 == 0:
            pf = get_profit_factor(conn, days=30)
            if pf < 1.5 and pf > 0:
                wins_30, losses_30, pnl_30 = get_rolling_stats(conn, days=30)
                wins_7,  losses_7,  pnl_7  = get_rolling_stats(conn, days=7)
                total_30 = wins_30 + losses_30
                wr_30    = (wins_30 / total_30 * 100) if total_30 > 0 else 0.0
                total_7  = wins_7 + losses_7
                wr_7     = (wins_7 / total_7 * 100) if total_7 > 0 else 0.0
                ...
                send_whatsapp(
                    ...
                    f"🎯 Doel:              >1.5\n"
                    f"{'⚠️ Onder doel' if pf < 1.5 else '✅ Boven doel'}\n\n"
                    f"📈 LAATSTE 30 DAGEN:\n"
                    ...
                    f"• PnL:    €{pnl_7:.2f}\n\n"
                    f"💡 WAT DIT BETEKENT:\n"
                    f"Bot verdient minder dan het verliest.\n"
                    f"Overweeg hogere score drempel of\n"
                    f"minder risicovolle setups.\n\n"
                    f"🤖 BOT LOOPT GEWOON DOOR\n"
                    f"Stuur STOP als je wil pauzeren.\n\n"
...
def _log_shadow_outcome(
    ...
    pnl_eur    = (exit_price - entry) * qty if entry > 0 else 0.0
    setup_type = safe_str(trade.get("setup_type"))
    regime     = safe_str(trade.get("regime"))
    score      = safe_int(trade.get("score"))
    prebuy_id  = safe_str(trade.get("prebuy_id"))
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            VALUES (%s,'SHADOW',%s,NOW(),NOW(),%s,%s,%s,%s,%s,%s,NOW(),NOW(),NOW())
            ON CONFLICT (trade_key) DO UPDATE SET
                outcome    = EXCLUDED.outcome,
                pnl_eur    = EXCLUDED.pnl_eur,
                exit_time  = NOW(),
...
def run_monitor_once(target_symbol: Optional[str] = None) -> None:
    ...
    1. Check bot state
    2. Verwerk alle open live trades
    3. Verwerk alle open shadow trades
    4. Bewaar gewijzigde state
    """
    try:
        conn = db_connect()
    except Exception as e:
    ...
    try:
        # Bot state check — monitor bewaakt altijd open trades
        # ook als bot gestopt is. Alleen nieuwe BUYs worden geblokkeerd.
        ...
        # Daily stop loss check — bot loopt door, jij beslist via STOP
        _, _, daily_pnl = get_daily_pnl(conn)
        if daily_pnl <= -DAILY_STOP_LOSS_EUR and bot_active:
            wins_v, losses_v, _ = get_daily_pnl(conn)
            wins_7d, losses_7d, pnl_7d = get_rolling_stats(conn, days=7)
            total_v = wins_v + losses_v
            wr_v    = (wins_v / total_v * 100) if total_v > 0 else 0.0
            open_n  = len(state.get("positions", {})) if 'state' in dir() else 0
            pf_30d  = get_profit_factor(conn, days=30)

            log(f"Daily stop loss geraakt: €{daily_pnl:.2f}")
            send_whatsapp(
                f"🛑 DAGBUDGET BEREIKT\n"
                f"{'─' * 30}\n\n"
                f"💶 Verlies vandaag:  €{abs(daily_pnl):.2f}\n"
                f"📊 Dagbudget limiet: €{DAILY_STOP_LOSS_EUR:.2f}\n\n"
                f"📈 VANDAAG:\n"
                f"• Wins:    {wins_v} trades\n"
                f"• Losses:  {losses_v} trades\n"
                f"• Win rate: {wr_v:.1f}%\n\n"
                f"📊 LAATSTE 7 DAGEN:\n"
                f"• Wins:    {wins_7d} | Losses: {losses_7d}\n"
                f"• PnL:     €{pnl_7d:.2f}\n\n"
                ...
                f"(worden wel bewaakt)\n\n"
                f"🤖 BOT LOOPT GEWOON DOOR\n"
                f"Dagbudget bereikt — bot handelt\n"
                f"automatisch verder.\n"
                f"Stuur STOP als je wil pauzeren.\n\n"
                f"Commands: STOP | STATUS | TRADES"
            )
        ...
        for symbol in symbols:
            ...
            # Coin blacklist check
            if is_coin_blacklisted(conn, symbol):
                log(f"⚫ {symbol} op blacklist — toch bewaken (al open)")
        ...
        log(f"✅ Monitor run klaar — {len(symbols)} live, {len(shadow_symbols if 'shadow_symbols' in dir() else [])} shadow")
...
def run_monitor_loop() -> None:
    """Continue monitor loop — draait op Render als background worker."""
    log("=" * 50)
    ...
    log(f"Cooldown:     {COIN_COOLDOWN_HOURS}u")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--once",
        action="store_true",
        help="Één run uitvoeren en stoppen",
    )
    parser.add_argument(
        "--symbol",
        type=str,
