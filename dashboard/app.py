...
# ============================================================
# SESSION STATE  alle keys genitialiseerd bij opstarten
...
LOSS_A10 = "rgba(255,17,51,0.10)"
LOSS_A20 = "rgba(255,17,51,0.20)"
LOSS_A35 = "rgba(255,17,51,0.35)"
LIVE_A10 = "rgba(0,229,255,0.10)"
LIVE_A20 = "rgba(0,229,255,0.20)"
...
BORDER   = "rgba(0,229,255,0.10)"   # standaard border
...
def chart_btc_correlation_bar(df: pd.DataFrame) -> go.Figure:
    ...
    color_map = {"BULL":"#00ff41","BEAR":"#ff1144","RANGE":"#00e5ff"}
...
def chart_pnl_histogram(df: pd.DataFrame) -> go.Figure:
    ...
    fig = go.Figure(go.Histogram(
        ...
        nbinsx=20,
        marker=dict(
            color=["#00ff41" if v >= 0 else "#ff1144" for v in euros],
            line=dict(width=0),
        ),
        opacity=0.8,
    ))
...
def chart_donut(win_pct: float, net_eur: float, title: str = "Win Rate", n_trades: int = -1, n_wins: int = 0, n_losses: int = 0) -> go.Figure:
    ...
    # 0.15%    zeer subtiele gedempte kleur (nauwelijks zichtbaar)
    # 530%     zwakke gedempte kleur
    # 3050%    matige kleur (oranje/geel als waarschuwing)
    # 5060%    groene kleur
    ...
    fig.add_trace(go.Pie(
        ...
        sort=False,
        textinfo="none",
        marker=dict(colors=["rgba(0,229,255,0.05)"], line=dict(width=0)),
        showlegend=False,
        hoverinfo="skip",
    ))
...
def chart_trade_detail(row: pd.Series) -> go.Figure:
    ...
    fig.update_layout(
        ...
        yaxis=dict(gridcolor="rgba(0,229,255,0.06)"),
    )
    return fig
...
def chart_r_distributie(df: pd.DataFrame) -> go.Figure:
    ...
    fig.add_trace(go.Histogram(
        ...
        marker_color="#00ff41", opacity=0.8,
    ))
    fig.add_vline(x=0, line_dash="dash", line_color="#00e5ff", line_width=1)
...
def chart_score_winrate_correlatie(df: pd.DataFrame) -> go.Figure:
    ...
    kleuren = ["#ff1144" if w < 45 else "#00e5ff" if w < 55 else "#00ff41"
               for w in grp["wr"]]
    fig = go.Figure(go.Bar(
...
def chart_fee_impact(df: pd.DataFrame) -> go.Figure:
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(name="Bruto Winst",  x=grp["maand"], y=grp["bruto_winst"],  marker_color="#00ff41"))
    fig.add_trace(go.Bar(name="Bruto Verlies",x=grp["maand"], y=-grp["bruto_verlies"],marker_color="#ff1144"))
    fig.add_trace(go.Bar(name="Fees",         x=grp["maand"], y=-grp["fees"],         marker_color="#00e5ff"))
...
def chart_config_timeline(config_df: pd.DataFrame) -> go.Figure:
    ...
    palette = ["#00e5ff","#ff1144","#00ff41","#00e5ff","#00e5ff","#00ff41","#00e5ff"]
...
def chart_trade_flow_funnel(flow: Dict) -> go.Figure:
    ...
    fig = go.Figure(go.Funnel(
        ...
        marker_color=["#00e5ff","#00e5ff","#00ff41","#00ff41"],
    ))
...
def chart_audit_gauge(score: int) -> go.Figure:
    ...
    kleur = "#00ff41" if score >= 90 else "#00e5ff" if score >= 70 else "#ff1144"
    fig = go.Figure(go.Indicator(
        mode="gauge+number+delta",
        value=score,
        domain={"x":[0,1],"y":[0,1]},
        title={"text":"Systeem Gezondheid","font":{"size":14,"color":"#4e5a7a"}},
        delta={"reference":90,"increasing":{"color":"#00ff41"},"decreasing":{"color":"#ff1144"}},
        gauge={
...
def render_coach_monitor_page() -> None:
    ...
    with tab_config:
        ...
        if config_df.empty:
            ...
            st.download_button(" Download config log", csv, "config_log.csv", "text/csv")

    #  ANOMALIEN 
    with tab_anom:
        anom_df = load_coach_anomalieen(14)
        if anom_df.empty:
            st.success(" Geen anomalien gedetecteerd in de afgelopen 14 dagen.")
        else:
            ...
            for _, row in anom_df.iterrows():
                ...
                st.markdown(
                    ...
                    f'{"#ff1144" if ernst=="KRITIEK" else "#00e5ff" if ernst=="HOOG" else "#555"}'
    ...
    with tab_regime:
        ...
        if regime_df.empty:
            ...
            for _, row in regime_df.iterrows():
                ...
                kleur_oud  = "#ff1144" if "BEAR" in oud  else "#00ff41" if "BULL" in oud  else "#00e5ff"
                kleur_nieuw= "#ff1144" if "BEAR" in nieuw else "#00ff41" if "BULL" in nieuw else "#00e5ff"
                st.markdown(
...
def render_coin_cluster_widget() -> None:
    ...
    """
    st.markdown("** Coin Clusters (ai_coach)**")
    try:
        conn = get_db_conn()
        if conn is None:
...
def render_scan_efficiency_widget() -> None:
    ...
    try:
        ...
        items = [
            ...
            ("Signalen",         data.get("totaal_signalen", 0), "#00e5ff"),
...
def render_shadow_live_brug_widget() -> None:
    ...
    try:
        ...
        diff = s_wr - l_wr
        diff_kleur = "#00ff41" if abs(diff) <= 10 else "#ff1144"
    ...
    for i, check in enumerate(versheid_checks):
        with cols[i]:
            try:
                ...
                kleur= "#00ff41" if ok else "#ff1144"
...
def render_controls_page() -> None:
    ...
    if bot_actief and not bot_gepauz:
        ...
        status_label  = " LIVE TRADING GEPAUZEERD"
    ...
    with btn_c2:
        if st.button(" PAUZEER", use_container_width=True, disabled=not bot_actief or bot_gepauz):
            if _set_bot_state_key("bot_paused", "true"):
                st.warning(" Live trading gepauzeerd  bot systeem draait door")
                st.rerun()

    with btn_c3:
        if st.button(" HERVAT", use_container_width=True, disabled=not bot_gepauz):
            if _set_bot_state_key("bot_paused", "false"):
                st.success(" Live trading hervat")
                st.rerun()
    ...
    #  PARAMETER SLIDERS 
    st.markdown("** Parameters direct aanpassen**")
    st.caption("Wijzigingen worden direct in bot_state opgeslagen n gelogd in coach_config_log.")
...
def _render_extra_analyse_tabs(df: pd.DataFrame, real_df: pd.DataFrame) -> None:
    ...
    tab_r, tab_sc, tab_fee, tab_cfg = st.tabs([
        " R-Distributie", " Score Correlatie", " Fee Impact", " Config Timeline",
    ])
...
def render_live_monitor() -> None:
    ...
    st.markdown(
        ...
        f'<span class="top-status-chip {bot_chip}">BOT: {bot_lbl}</span>'
    ...
    if markt_txt:
        st.markdown(
            f'<div style="background:rgba(0,229,255,0.04);border:1px solid var(--border);'
...
def render_sidebar() -> None:
    ...
    st.markdown('<div class="nav-header" style="margin-top:8px;"> Kalender & Correlatie</div>', unsafe_allow_html=True)
...
def render_trade_list(df: pd.DataFrame, title: str = "Trades") -> None:
    ...
    for _, row in df.head(12).iterrows():
        ...
        color    = "#00ff41" if outcome == "WIN" else "#ff1144" if outcome == "LOSS" else "#4e5a7a"
        label    = (
...
def render_trade_detail(row: Optional[pd.Series]) -> None:
    ...
    outcome_color = "#00ff41" if outcome == "WIN" else "#ff1144" if outcome == "LOSS" else "#4e5a7a"
...
def render_top_bar(
    ...
    <div class="topbar">
        ...
        <div class="top-status-row">
            <span class="top-status-chip {bot_chip_cls}">{bot_label}</span>
...
def render_trade_page(
    ...
    st.download_button(f" Download {title} CSV", csv, f"{page_name}_trades.csv", "text/csv", use_container_width=True)
...
def render_settings_page() -> None:
    ...
    st.markdown('<div class="panel">', unsafe_allow_html=True)
    st.markdown('<div class="section-title"> Bot Instellingen</div>', unsafe_allow_html=True)
    st.markdown(
        '<div class="section-subtitle">'
...
def render_overall_winloss_bar(history_df: pd.DataFrame) -> None:
    ...
    for col, key, emoji, label in [
        ...
        with col:
            st.markdown(
                f'<div style="background:rgba(0,229,255,0.04);'
                f'border:1px solid rgba(0,229,255,0.07);'
...
def render_open_positions_page() -> None:
    ...
    with m2:
        st.markdown(metric_card("Genvesteerd", f"{total_invested:.2f}", "totaal open", "blue"), unsafe_allow_html=True)
    with m3:
        float_pct = (total_float / max(total_invested, 0.001)) * 100
        st.markdown(metric_card("Float %", f"{float_pct:+.2f}%", "rendement op genvesteerd", "orange" if abs(float_pct) < 2 else "green" if float_pct >= 0 else "red"), unsafe_allow_html=True)
    ...
    for pos in positions:
        ...
        pnl_color = "#00ff41" if is_profit else "#ff1144"
        r_color   = "#00ff41" if float_r >= 0 else "#ff1144"
...
def render_analyse_page(history_df: pd.DataFrame, real_df: pd.DataFrame) -> None:
    ...
    tab_dd, tab_rolling, tab_streak, tab_hold, tab_hist, tab_freq = st.tabs([
        ...
        " Houdtijd", " P&L Verdeling", " Frequentie"
    ])
    ...
    for day_num in range(1, days_in_month + 1):
        ...
        if pnl_val is None or n_trades == 0:
            ...
            cells.append(
                ...
                f'style="background:rgba(0,255,65,0.{intensity:02d});{vandaag_border}" '
                f'title="{tooltip}">'
                f'{day_num}'
                f'<div style="font-size:9px;color:rgba(0,255,65,0.9);">+{pnl_val:.1f}R</div>'
                f'</div>'
            )
            month_wins += 1
            month_r    += pnl_val
            ...
            cells.append(
                ...
                f'style="background:rgba(255,17,51,0.{intensity:02d});{vandaag_border}" '
                f'title="{tooltip}">'
                f'{day_num}'
                f'<div style="font-size:9px;color:rgba(255,17,51,0.9);">{pnl_val:.1f}R</div>'
                f'</div>'
            )
            month_losses += 1
            month_r      += pnl_val
...
Wees altijd positief en constructief. Leg exact uit wat de status betekent.""".strip()
    ...
    try:
        ...
        return " Timeout  probeer een kortere vraag."
    except Exception as e:
...
def render_coach_chat_page(history_df: pd.DataFrame, real_df: pd.DataFrame) -> None:
    ...
    pnl_kleur  = "#00ff41" if context.get("pnl_vandaag", 0) >= 0 else "#ff1144"
    ...
    with wis_col:
        if st.button(" Wis", use_container_width=True, key="coach_wis"):
...
def render_scan_efficiency_widget() -> None:
    ...
    kleur_conv = "#22c55e" if conv >= 0.3 else "#00e5ff" if conv >= 0.1 else "#94a3b8"
    ...
    st.markdown(
        ...
        <div style='display:flex;gap:16px;flex-wrap:wrap;'>
          ...
          <span style='color:#94a3b8;font-size:12px;'> {duur:.0f}s</span>
...
def render_coin_clusters_widget() -> None:
    ...
    st.markdown(
        "<div style='color:#94a3b8;font-size:11px;margin-bottom:6px;'> Coin Clusters</div>",
        unsafe_allow_html=True,
    )
...
def render_rollback_widget() -> None:
    ...
    st.markdown(
        ...
        <div style='color:#f59e0b;font-size:11px;margin-bottom:6px;'> Parameter Rollbacks (7d)</div>
        {"".join(items)}
        </div>""",
        unsafe_allow_html=True,
    )
...
def render_uitgebreide_monitor_pagina() -> None:
    ...
    st.markdown(
        "<div style='color:#94a3b8;font-size:11px;margin-top:8px;'> Gradient Score Optimizer</div>",
        unsafe_allow_html=True,
    )
    try:
        df_grad = run_query("""
        ...
        if isinstance(df_grad, pd.DataFrame) and not df_grad.empty:
            ...
            kleur    = "#22c55e" if aanbev == "HOUD" else "#00e5ff"
            st.markdown(
