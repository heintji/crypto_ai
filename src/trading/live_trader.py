# live_trader.py
# ============================================================
# Crypto AI Bot ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Live Trader v3.0
# ============================================================
# Voert echte BUY en SELL orders uit op Bitvavo.
# Gebruikt Bitvavo API voor live orders, Binance voor data.
#
# V3.0 PATTERN ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ identiek aan alle andere bestanden:
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ safe_rollback() overal
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ db_connect() retries=3 + autocommit=False
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ conn=None voor try/finally + finally conn.close()
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ _bitvavo_request() geen retry op 4xx client errors
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ isinstance(s, dict) check voor JSON state files
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ WhatsApp rate limiting per fouttype
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Model: claude-sonnet-4-6
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ log_naar_bot_state() schrijft naar bot_state tabel
#
# NIEUWE FEATURES V3.0:
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ shadow_buy() ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ shadow trade parallel aan elke live BUY
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ bereken_positiegrootte_kelly() ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Half Kelly Criterion sizing
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ validate_atr_stop() ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ATR-based stop validatie
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ log_naar_bot_state() ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ live_trader_busy/last_action/last_ts/error
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ get_account_snapshot() ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ volledige Bitvavo portfolio snapshot
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ check_min_order_size() ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ waarschuwt bij order onder Bitvavo minimum
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ get_coin_stats() ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ win rate / R / profit factor per coin
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ bereken_pnl_nauwkeurig() ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ PnL incl. fee_buy + fee_sell
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ log_trade_event() ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ elke actie naar coach_events voor ai_coach
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ load_shadow_state() / save_shadow_state() ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ shadow state helpers
#
# BUGS GEFIXED vs v2.0:
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ HMAC signing ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ digestmod=hashlib.sha256
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ get_tradable_markets() publiek
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ price=0 bug via fills fallback
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ sslmode="require" op DB connectie
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Auto mode: live eerst, paper als fallback
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Geen automatische pauze ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ bot gaat altijd door
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ buy_eur/sell: conn=None + finally conn.close()
#   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ _bitvavo_request: geen retry op alle 4xx (was alleen 401/403)
# ============================================================
...
# ============================================================
# ENV ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ identiek aan alle andere bestanden
# ============================================================
...
# ============================================================
# FASE 1 LIMIETEN ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ identiek aan alle andere bestanden
# ============================================================
...
# Fee + slippage ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ identiek aan alle bestanden
...
# ATR parameters ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ identiek aan trade_monitor
...
# WhatsApp rate limiting per fouttype ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ voorkomt spam bij herhaalde fouten
_WA_LAST_SENT: Dict[str, float] = {}
_WA_COOLDOWN_SECS = 300  # 5 minuten per fouttype


# ============================================================
# BASIS HELPERS ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ identiek aan alle andere bestanden
# ============================================================
...
def safe_rollback(conn) -> None:
    """
    Rollback zonder exception ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ v3.0 pattern.
...
# ============================================================
# WHATSAPP ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ identieke implementatie + rate limiting v3.0
# ============================================================
def send_whatsapp(message: str) -> bool:
    """
    Stuurt WhatsApp bericht via Twilio.
    Identieke implementatie in alle bestanden.
    Alleen voor kritieke meldingen ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ geen spam per trade.
    """
    ...
    try:
        ...
        if resp.status_code in (200, 201):
            log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ WhatsApp verzonden ({len(message)} tekens)")
            return True
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ WhatsApp {resp.status_code}: {resp.text[:200]}")
        return False
    except Exception as e:
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ WhatsApp exception: {type(e).__name__}: {e}")
        return False


def send_whatsapp_rate_limited(message: str, key: str = "default") -> bool:
    """
    WhatsApp met rate limiting per fouttype ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ v3.0 pattern.
    ...
    if now - last < _WA_COOLDOWN_SECS:
        ...
        log(f"ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂµ WhatsApp rate limited ({key}) ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ nog {remaining}s wachten")
        return False
...
# ============================================================
# CLAUDE HEALTH MONITORING ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ model: claude-sonnet-4-6
# ============================================================
def _claude_analyse(prompt: str, max_tokens: int = 300) -> str:
    """Claude API aanroep ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ identiek aan alle bestanden."""
...
def report_error(
    ...
    Rate limited per fouttype ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ geen spam.
...
""".strip()
    ...
    # Rate limited per severity+function combinatie ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ geen spam
    wa_key = f"error_{severity}_{function.replace('.', '_')}"
    send_whatsapp_rate_limited(
        f"ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¨ LIVE TRADER FOUT ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ {severity}\n"
        f"{'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ' * 30}\n\n"
        f"ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ Functie:     {function}\n"
        f"ÃÂÃÂ°ÃÂÃÂÃÂÃÂªÃÂÃÂ Coin:        {symbol or 'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ'}\n"
        f"ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ Open trades: {open_trades}\n"
        f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Fout:       {type(error).__name__}\n\n"
        f"ÃÂÃÂ°ÃÂÃÂÃÂÃÂ§ÃÂÃÂ  Claude:\n{uitleg}\n\n"
        f"ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ WAT TE DOEN:\n"
        ...
        f"ÃÂÃÂ°ÃÂÃÂÃÂÃÂ¤ÃÂÃÂ BOT PROBEERT DOOR TE GAAN\n"
...
Entry:     {entry:.6f} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Exit: {exit_price:.6f}
PnL:       ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{pnl_eur:.4f}
...
CONFIGURATIE:
- BITVAVO_API_KEY:    {'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ aanwezig' if BITVAVO_API_KEY else 'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ONTBREEKT'}
- BITVAVO_API_SECRET: {'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ aanwezig' if BITVAVO_API_SECRET else 'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ONTBREEKT'}
- DATABASE_URL:       {'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ aanwezig' if DATABASE_URL else 'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ONTBREEKT'}
- MAX_PER_TRADE_EUR:  ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{MAX_PER_TRADE_EUR:.2f}
- DAILY_STOP_LOSS:    ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{DAILY_STOP_LOSS_EUR:.2f}
...
2. Zijn er potentiÃÂÃÂÃÂÃÂ«le problemen?
3. Aanbevelingen?
""".strip()
...
# ============================================================
# DATABASE ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ v3.0: retries=3 + autocommit=False
# ============================================================
def db_connect(retries: int = 3):
    """
    DB verbinding met sslmode=require ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ v3.0 pattern.
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if attempt < retries:
                wait = 2 ** attempt
                log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ DB connect poging {attempt}/{retries} mislukt, wacht {wait}s: {e}")
                time.sleep(wait)
    raise RuntimeError(f"DB connect mislukt na {retries} pogingen: {last_err}")


# ============================================================
# BOT STATE ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ identiek aan alle bestanden
# ============================================================
...
def set_bot_state(conn, key: str, value: str) -> None:
    try:
        ...
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ set_bot_state fout: {e}")


def _set_bot_state_multi(conn, kvs: Dict[str, str]) -> None:
    """Meerdere bot state waarden in ÃÂÃÂÃÂÃÂ©ÃÂÃÂÃÂÃÂ©n transactie ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ efficiÃÂÃÂÃÂÃÂ«nter."""
    try:
        with conn.cursor() as cur:
        ...
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ _set_bot_state_multi fout: {e}")
        safe_rollback(conn)
...
def log_naar_bot_state(
    ...
    """
    Schrijft live trader status naar bot_state tabel ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ v3.0.
    Wordt door app.py gelezen voor dashboard display.

    Keys die worden geschreven:
      live_trader_last_action  ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ laatste actie (bv. "BUY ETHUSDT @ ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬2500")
      live_trader_last_ts      ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ timestamp ISO (voor Render Services Monitor)
      live_trader_busy         ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ "true" als trade bezig is
      live_trader_last_pnl     ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ PnL van laatste gesloten trade
      live_trader_error        ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ laatste foutmelding (leeg = OK)
    """
    conn = None
    try:
        conn = db_connect()
        ...
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ log_naar_bot_state fout: {e}")
    finally:
        if conn:
            conn.close()
...
# ============================================================
# LIMIETEN CHECK ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ identiek aan whatsapp_webhook.py
# Bot stopt NOOIT automatisch ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ jij beslist via STOP
# ============================================================
...
def get_open_real_trades_count(conn) -> int:
    """Telt open echte trades ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ state file eerst, dan DB als fallback."""
    try:
        state = load_state()
...
def get_consecutive_losses(conn) -> int:
    """Opeenvolgende verliezen ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ identiek aan alle bestanden."""
    try:
        with conn.cursor() as cur:
            cur.execute("""
...
def check_trading_limits(conn) -> Tuple[bool, str]:
    ...
    if not is_bot_active(conn):
        return False, "Bot GESTOPT ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ stuur START"
    ...
    # Daily stop loss: alleen informeren ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ bot gaat door
    ...
    if daily_pnl <= -daily_stop:
        log(
            f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¹ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Dagbudget bereikt: ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{daily_pnl:.2f} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ "
    ...
    # Consecutive losses: alleen informeren ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ bot gaat door
    consecutive = get_consecutive_losses(conn)
    if consecutive >= MAX_CONSECUTIVE_LOSSES:
        log(
            f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¹ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ {consecutive}x verlies op rij ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ "
            f"bot gaat door (jij beslist via STOP)"
        )
...
# ============================================================
# BITVAVO UNIVERSE FILTER ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ publiek + gecached
# ============================================================
def get_tradable_markets() -> Set[str]:
    ...
    try:
        ...
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Bitvavo markets gecached: {len(tradable)} tradable")
        return tradable

    except Exception as e:
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Bitvavo markets fout: {e}")
        return _MARKETS_CACHE.get("markets") or set()


def symbol_to_market(symbol_usdt: str) -> Optional[str]:
    """
    ETHUSDT ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ETH-EUR als tradable op Bitvavo.
...
# ============================================================
# BITVAVO API ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ SIGNING + REQUEST
...
def _bitvavo_request(
    ...
    (Was alleen 401/403 ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ nu ook 400, 404, 422 etc.)
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            log(
                f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Bitvavo {method} {path} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ {err_code}: {err_msg} "
                f"(poging {attempt}/{retries})"
            )

            # v3.0: geen retry op ALLE 4xx client errors
            if 400 <= resp.status_code < 500:
                log(f"  Client error {resp.status_code} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ geen retry")
                return False, f"Client error {resp.status_code}: {err_msg}"

        except requests.exceptions.Timeout:
            log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Bitvavo timeout poging {attempt}/{retries}")
        except Exception as e:
            log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Bitvavo request fout poging {attempt}/{retries}: {e}")
...
def get_price_binance(symbol_usdt: str) -> Optional[float]:
    ...
    try:
        ...
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Binance prijs fout ({symbol_usdt}): {e}")
    return None
...
def get_price_bitvavo(market: str) -> Optional[float]:
    ...
    try:
        ...
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Bitvavo prijs fout ({market}): {e}")
    return None
...
# ============================================================
# LIVE STATE ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ file helpers
# v3.0: isinstance(s, dict) check ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ corrupt JSON auto-reset
# ============================================================
def load_state() -> Dict[str, Any]:
    ...
    try:
        with open(LIVE_STATE_PATH, "r", encoding="utf-8") as f:
            s = json.load(f)
        # v3.0: isinstance check ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ was ontbreekt in v2.0
        if not isinstance(s, dict):
            log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ live_state.json corrupt (type={type(s).__name__}) ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ reset")
            s = {}
    except Exception as e:
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ live_state.json laad fout: {e} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ reset")
        s = {}
...
# ============================================================
# SHADOW STATE ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ v3.0 nieuw
...
def load_shadow_state() -> Dict[str, Any]:
    ...
    try:
        with open(SHADOW_STATE_PATH, "r", encoding="utf-8") as f:
            s = json.load(f)
        if not isinstance(s, dict):
            log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ shadow_trades.json corrupt ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ reset")
            s = {}
    except Exception as e:
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ shadow_trades.json laad fout: {e} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ reset")
        s = {}
    s.setdefault("positions", {})
    s.setdefault("closed", [])
    return s
...
def shadow_buy(
    ...
    """
    Logt een shadow trade parallel aan elke live BUY ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ v3.0 nieuw.
    ...
    try:
        ...
        log(f"ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¤ Shadow BUY gelogd: {symbol} @ {entry:.6f}")
    except Exception as e:
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ shadow_buy fout ({symbol}): {e}")
...
def shadow_sell(
    ...
    """
    Sluit een shadow trade ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ aangeroepen vanuit sell().
    ...
    try:
        ...
        save_shadow_state(state)
        log(f"ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¤ Shadow SELL: {symbol} {outcome} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{pnl_eur:.4f}")
    except Exception as e:
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ shadow_sell fout ({symbol}): {e}")


# ============================================================
# ACCOUNT SNAPSHOT ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ v3.0 nieuw
# ============================================================
def get_account_snapshot() -> Dict[str, Any]:
    ...
    ok, data = _bitvavo_request("GET", "/balance")
    if not ok:
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Account snapshot mislukt: {data}")
        return {}
    ...
    try:
        ...
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Account snapshot: {len(snapshot['balances'])} coins, "
            f"EUR={snapshot['eur_available']:.2f}")
    except Exception as e:
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Snapshot opslaan fout: {e}")

    return snapshot


# ============================================================
# COIN STATS ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ v3.0 nieuw
# ============================================================
def get_coin_stats(conn, symbol: str) -> Dict[str, Any]:
    ...
    try:
        ...
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ get_coin_stats fout ({symbol}): {e}")
...
# ============================================================
# KELLY CRITERION ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ v3.0 nieuw
# ============================================================
def bereken_positiegrootte_kelly(
    ...
    """
    Berekent optimale positiegrootte via Half Kelly Criterion ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ v3.0 nieuw.
    ...
    Constraints:
      ...
      - Resultaat: min ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬0.50, max ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬5.00
    ...
    try:
        ...
        if n < 10:
            log(f"Kelly {symbol}: onvoldoende data (n={n} < 10) ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{base_eur:.2f}")
            return base_eur
        ...
        log(
            ...
            f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{result:.2f}"
        )
        return result

    except Exception as e:
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Kelly fout ({symbol}): {e}")
        return base_eur


# ============================================================
# ATR STOP VALIDATIE ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ v3.0 nieuw
# ============================================================
def validate_atr_stop(
    ...
    """
    Valideert en corrigeert ATR-based stop loss ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ v3.0 nieuw.
    ...
    if stop >= entry:
        ...
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Stop boven entry voor {symbol}: {stop:.6f} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ {corrected:.6f}")
        return corrected
    ...
    if atr and atr > 0:
        ...
        if stop > max_stop:
            log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Stop te krap voor {symbol}: {stop:.6f} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ {max_stop:.6f} (0.5x ATR)")
            return max_stop

        if stop < min_stop:
            log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Stop te wijd voor {symbol}: {stop:.6f} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ {min_stop:.6f} ({ATR_MULTIPLIER}x ATR)")
            return min_stop
    ...
    if stop > min_stop_pct:
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Stop te krap (geen ATR) voor {symbol}: {stop:.6f} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ {min_stop_pct:.6f}")
        return min_stop_pct
...
# ============================================================
# MIN ORDER SIZE CHECK ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ v3.0 nieuw
# ============================================================
def check_min_order_size(amount_eur: float) -> Tuple[bool, str]:
    """
    Controleert of order boven Bitvavo minimum zit ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ v3.0 nieuw.

    Bitvavo minimum is ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬5.00. Fase 1 limit is ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬0.50.
    Dit is een WAARSCHUWING, geen blokkade ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ
    ...
    van _bitvavo_request() ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ die wordt correct afgehandeld.
    """
    if amount_eur < BITVAVO_MIN_ORDER_EUR:
        msg = (
            f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Order ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{amount_eur:.2f} < Bitvavo minimum "
            f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{BITVAVO_MIN_ORDER_EUR:.2f} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ kan worden afgewezen"
        )
        log(msg)
        return False, msg
    return True, "OK"
...
# ============================================================
# PNL BEREKENING ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ v3.0 nieuw
# ============================================================
def bereken_pnl_nauwkeurig(
    ...
    """
    Berekent PnL nauwkeurig inclusief alle kosten ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ v3.0 nieuw.

    v2.0 had alleen fee_buy meegeteld ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ fee_sell ontbrak.
...
# ============================================================
# DB LOGGING ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ experience_trades + coach_events
# ============================================================
def log_trade_event(
    conn,
    symbol:     str,
    ...
    """
    Logt een trade event naar coach_events voor ai_coach ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ v3.0 nieuw.
    ...
    try:
        ...
        # coach_events tabel bestaat mogelijk nog niet ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ geen crash
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ log_trade_event fout ({symbol}/{event_type}): {e}")
        safe_rollback(conn)
...
def log_trade_open_to_db(
    ...
    try:
        ...
        conn.commit()
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ DB gelogd (OPEN): {symbol} entry={entry:.6f}")
        ...
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ log_trade_open_to_db fout ({symbol}): {e}")
        safe_rollback(conn)
...
def log_trade_close_to_db(
    ...
    try:
        ...
        conn.commit()
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ DB gelogd ({outcome}): {symbol} pnl=ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{pnl_eur:.4f}")
        ...
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ log_trade_close_to_db fout ({symbol}): {e}")
        safe_rollback(conn)


# ============================================================
# COIN FILTERS ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ identiek aan multi_coin_score en trade_monitor
# ============================================================
...
# ============================================================
# BUY ORDER ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Bitvavo market BUY helpers
# ============================================================
def place_market_buy_eur(
    ...
    log(f"ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¤ Bitvavo BUY: {market} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{amount_eur:.2f}")
    ...
    try:
        ...
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ BUY exception ({market}): {e}")
        return False, {"error": str(e)}
    if not ok:
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ BUY mislukt ({market}): {data}")
        return False, {"error": str(data)}
    if isinstance(data, dict) and "error" in data:
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ BUY error ({market}): {data}")
        return False, data
    ...
    log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ BUY uitgevoerd: {market} qty={qty:.6f} @ ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{price:.6f}")
    return True, data
...
def place_market_sell(
    ...
    fraction=1.0 ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ alles, fraction=0.40 ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ 40%
    """
    ...
    log(f"ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¤ Bitvavo SELL: {market} qty={sell_qty:.6f} ({fraction*100:.0f}%)")
    ...
    try:
        ...
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ SELL exception ({market}): {e}")
        return False, {"error": str(e)}
    if not ok:
        ...
        if _is_212:
            log(f"\u26d4 Error 212 ghost close: {market} waarde te klein — positie markeren als gesloten")
            return True, {
                "_parsed_price":    0.0,
    ...
    log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ SELL uitgevoerd: {market} qty={sold_qty:.6f} @ ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{price:.6f}")
    return True, data


# ============================================================
# HOOFD BUY FUNCTIE ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ v3.0
...
def buy_eur(
    ...
    """
    Voert een live BUY uit op Bitvavo ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ v3.0.

    Stappen:
    1.  Market ophalen (USDTÃÂÃÂ¢ÃÂÃÂÃÂÃÂEUR)
    ...
    try:
        ...
        if is_coin_blacklisted(conn, symbol):
            ...
            log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ« {msg}")
        ...
        if eur_balance < amount_eur:
            msg = f"Onvoldoende EUR: ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{eur_balance:.2f} < ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{amount_eur:.2f}"
            log_naar_bot_state(f"BUY geblokkeerd: {msg}", busy=False)
            return False, msg
        ...
        if not ok:
            ...
            send_whatsapp_rate_limited(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ BUY FOUT: {symbol}\nReden: {err_msg[:100]}\nSaldo: ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{eur_balance:.2f}", key=f"buy_fout_{symbol}")
        ...
        if entry <= 0 or qty <= 0:
            log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Prijs/qty ongeldig na BUY ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ fallback ticker")
        ...
        log_naar_bot_state(
            f"BUY {symbol} @ ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{entry:.6f}",
            busy=False,
        )

        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Live BUY: {symbol} @ ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{entry:.6f} qty={qty:.6f} stop={stop:.6f}")
        send_whatsapp_rate_limited(
            f"ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¢ TRADE GEOPEND: {symbol}\n"
            f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ\n\n"
            f"ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ° Bedrag:  ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{amount_eur:.2f}\n"
            f"ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ Entry:   ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{entry:.6f}\n"
            f"ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ Stop:    ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{stop:.6f}\n"
            f"ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¯ Target:  ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{target:.6f}\n",
            key=f"buy_{symbol}"
        )
        return True, f"BUY {symbol} @ ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{entry:.6f}"
...
# ============================================================
# HOOFD SELL FUNCTIE ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ v3.0
...
def sell(
    ...
    """
    Voert een live SELL uit op Bitvavo ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ v3.0.

    fraction=1.0  ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ verkoop alles (stop loss, structuur break, max hold)
    fraction=0.40 ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ partial sell (40% na eerste keer >1R)

    Wordt aangeroepen door trade_monitor.py via _execute_sell().

    v3.0 verbeteringen:
    - bereken_pnl_nauwkeurig() ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ fee_sell was ontbreekt in v2.0
    ...
    if fraction >= 1.0:
        try:
            ...
            if bal <= 0:
                send_whatsapp_rate_limited(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ SELL GEBLOKKEERD: {symbol}\nGeen {coin} op Bitvavo", key=f"sell_check_{symbol}")
    ...
    try:
        ...
        if not ok:
            ...
            # â sluit positie als ghost zodat bot stopt met proberen
            is_212 = "212" in str(err_raw) or (isinstance(err_raw, dict) and err_raw.get("errorCode") == 212)
            if is_212:
                log(f"\u26d4 SELL error 212 ({symbol}): waarde te klein voor Bitvavo â positie gesloten als MIN_ORDER_GHOST")
                ...
                # Coins blijven in wallet â user moet handmatig verkopen zodra waarde herstelt
                return {
                    "ok":          True,
        ...
        if fraction >= 1.0:
            ...
            # Partial sell ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ update resterende positie
        ...
        # Claude trade analyse ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ alleen bij WIN of forse verlies
        claude_txt = ""
        hold_min   = 0.0
        try:
        ...
        try:
            ...
            log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ DB log fout bij SELL ({symbol}): {e}")

        # bot_state updaten voor dashboard
        log_naar_bot_state(
            f"SELL {symbol} {outcome} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{pnl_eur:.4f}",
            busy=False,
            pnl_eur=pnl_eur,
        )

        icon = "ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ" if outcome == "WIN" else "ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ"
        log(
            f"{icon} SELL {symbol}: {outcome} "
            f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{pnl_eur:.4f} | "
...
# ============================================================
# MAIN ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ configuratie check + Bitvavo test
# ============================================================
if __name__ == "__main__":
    log("=" * 60)
    log("Live Trader v3.0 ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ configuratie check")
    log("=" * 60)
    log(f"Database:        {'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ' if DATABASE_URL       else 'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ONTBREEKT'}")
    log(f"Bitvavo Key:     {'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ' if BITVAVO_API_KEY    else 'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ONTBREEKT'}")
    log(f"Bitvavo Secret:  {'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ' if BITVAVO_API_SECRET else 'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ONTBREEKT'}")
    log(f"Twilio:          {'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ' if TWILIO_ACCOUNT_SID else 'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ niet ingesteld'}")
    log(f"Claude API:      {'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ' if ANTHROPIC_API_KEY  else 'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ niet ingesteld'}")
    log(f"Max trade:       ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{MAX_PER_TRADE_EUR:.2f}")
    log(f"Daily stop:      ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{DAILY_STOP_LOSS_EUR:.2f}")
    ...
    if BITVAVO_API_KEY and BITVAVO_API_SECRET:
        ...
        eur = get_eur_balance()
        log(f"EUR balance: ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¬{eur:.2f}")
    ...
    log("ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Live Trader v3.0 configuratie check klaar")


def main_loop():
    """Hoofd trading loop ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ draait continu."""
    log("ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ Live Trader main loop gestart")
    conn = None
    try:
        conn = db_connect()
    except Exception as e:
        log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ DB verbinding mislukt: {e}")
        return
    ...
    while True:
        try:
            ...
            for row in rows:
                ...
                log(f"ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ Pending: {symbol} score={score}")
                ...
                if symbol in _state_check.get("positions", {}):
                    ...
                    conn.commit()
                    continue
                # Stop-waarde check: sell moet altijd mogelijk zijn (Bitvavo min â¬5)
                ...
                if ok:
                    ...
                    conn.commit()
                    log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ BUY uitgevoerd: {symbol}")
                else:
            ...
            log(f"ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Loop fout: {e}")
            try:
                conn.rollback()
            except Exception:
                pass
