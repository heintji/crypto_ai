...
def _sessie_timing() -> str:
    h = datetime.datetime.utcnow().hour
    if 8 <= h < 14: return "EU_US"
    elif 14 <= h < 22: return "US"
    elif 0 <= h < 8: return "AZIATISCH"
    return "NACHT"
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            if cur.rowcount == 0:
                ...
                dag_van_de_week, uur_utc, sessie_timing,
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            ON CONFLICT (trade_key) DO UPDATE SET
                ...
                gemiste_winst_r = EXCLUDED.gemiste_winst_r,
                hold_time_hours = EXCLUDED.hold_time_hours,
                closed_at       = EXCLUDED.closed_at,
                dag_van_de_week = EXCLUDED.dag_van_de_week,
                uur_utc         = EXCLUDED.uur_utc,
                sessie_timing   = EXCLUDED.sessie_timing,
                exit_time       = NOW(),
                updated_at      = NOW()
            """, (
                trade_key, symbol,
                ...
                score,
                exit_reden,
                round(min_price, 8) if min_price else None,
                round(max_price, 8) if max_price else None,
                round(mae_r, 3), round(mfe_r, 3),
                round(max(0.0, mfe_r - exit_r), 3),
                round(hold_min / 60.0, 2),
