...
# =========================================
# PATHS (altijd relatief aan project-root)
# =========================================
DATA_DIR = os.path.join(PROJECT_ROOT, "data")
PENDING_PATH = os.path.join(DATA_DIR, "pending_approvals.json")

# =========================================
# SECURITY (internal endpoint)
# =========================================
...
# =========================================
# INTERNAL ENDPOINT: Pre-BUY inbox
# multi_coin_score -> POST hierheen
# =========================================
...
def internal_prebuy():
    try:
        ...
        # minimal validation
        prebuy_id = str(payload.get("id", "")).strip()
        coin = str(payload.get("coin", "")).strip()
        if not prebuy_id or not coin:
            return jsonify({"ok": False, "reason": "missing_id_or_coin"}), 400

        pending = load_pending()

        # voorkom dubbele insert op zelfde id
        if any(str(x.get("id", "")).strip() == prebuy_id for x in pending):
            return jsonify({"ok": True, "status": "exists"}), 200

        # status afdwingen
        payload["status"] = STATUS_PENDING

        # expiry fallback (4 uur)
        ...
        return jsonify({"ok": True, "status": "stored", "count": len(pending)}), 200
...
def whatsapp():
    try:
        ...
        if not coin:
            return twiml("⚠️ Pre-BUY mist 'coin'. Check payload.")
...
if __name__ == "__main__":
    # lokaal testen: python whatsapp_webhook.py
    # Render start via Start Command (gunicorn) — maar dit laten we staan voor jouw test
